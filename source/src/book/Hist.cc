...
void Handle<RH<D, T, STAT ...>>::fillN(
	const std::span<typename Handle<RH<D, T, STAT ...>>::CoordArray_t>& x,
...
template<int D, typename T, template<int, class>class ... STAT>
EntryMultiShared<RH<D, T, STAT ...>>::EntryMultiShared(const Context& context)
	: _context { context },
		_fillMgr { std::make_shared<RHistConcurrentFillManager<RH<D, T, STAT ...>>>(*context.mem->at<RH<D, T, STAT ...>>(0))},
		_fillers ( 0 )
		{}
