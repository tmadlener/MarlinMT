...
namespace marlin{
    ...
    void XMLParser::parse(){
        ...
        _current->add( "ActiveProcessors", activeProcs ) ;
        _current->add( "ProcessorConditions", procConds ) ;
        ...
        globalParameters->add( "AvailableProcessors", availableProcs )  ;
    ...
    if( cmdlinevalues.compare( "" ) != 0 ){
      ...
      try {
        ...
        throw e ;
            ...
            _current->add( "_marlin.lcioInType", lcioInTypes  ) ;
            _current->add( "_marlin.lcioOutType", lcioOutTypes  ) ;
        }
