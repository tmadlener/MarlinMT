  ...
  double          _speedup {0.} ;
	friend std::istream& operator>>(std::istream& is, Entry& entry) {
		...
		is	
			>> entry._ncores >> c 
			>> entry._crunchTime  >> c
			>> entry._crunchSigma  >> c
			>> entry._nHist >> c
			>> entry._nFills >> c
			>> entry._nBins >> c
			>> entry._serialTime >> c 
			>> entry._parallelTime  >> c
			>> entry._speedup ;
		return is;
	}
...
void PlotScaling( const std::string &fname, const std::string &title = "", bool save = false ) {
  ...
  // std::map<std::size_t, std::vector<Entry>>   coreToEntryMap ;
	std::map<std::pair<std::size_t, std::size_t>, std::vector<Entry>>
		crunchNHistToEntryMap {} ;
	
	Entry entry;
	std::string header;
	std::getline(file, header);
  while( file >> entry ) {
		crunchNHistToEntryMap[{entry._crunchTime, entry._nHist}].push_back(entry);
  }
	TCanvas *speedupVsCoreCanvas = 
		new TCanvas( 
			"speedupVsCoreCanvas", 
			"Speedup Vs #Core", 800, 800 ) ;
	...
	for (int i = 0; i < 300; i+=100){ 
		...
		int color = 0 ;
		size_t maxCores = 0 ;
		for( auto ctIter : crunchNHistToEntryMap ) {
			
			auto crunchTime = ctIter.first.first ;
			if ( crunchTime != i ) { continue; }
			auto nHist = ctIter.first.second;
			std::stringstream title ; title << nHist << "  " << crunchTime ;
